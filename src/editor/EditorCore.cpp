...
EditorCore::EditorCore(Vec2 &windowSize, gpu::GPUContext *context)
    : cells(12, 12), tileSize(windowSize.y / cells.x, windowSize.y / cells.y),
      gridRenderer(context)
// engine(windowSize,tileSize)
{
  /*
info.guiIndex = 1;
info.currentTab = 0;
...
*/
  ...
  gridRenderer.populateBuffer(context, windowSize, cells, tileSize)
...
void EditorCore::draw(SDL_GPURenderPass *renderpass) {

  gridRenderer.draw(renderpass);
  /*
...
void EditorCore::events(SDL_Event &windowEvent) {
  // info.buttonInfo = {};
  /*
    static int temp = 0;
    switch (sfEvent.type) {
    ...
    case sf::Event::KeyReleased:
      ...
      if (((sfEvent.key.code - sf::Keyboard::Num1) |
           (sf::Keyboard::Num9 - sfEvent.key.code)) >= 0) {
        info.currentTab =
            sfEvent.key.code - static_cast<uint32_t>(sf::Keyboard::Num1);
        if (info.activeInventory) {
...
void EditorCore::resetButtonInfo() { // info.buttonInfo = {};
}
