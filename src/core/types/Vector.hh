...
template <typename T> struct Vec<2, T> {
  ...
  constexpr bool operator==(Vec const &v) const { return x == v.x && y == v.y; }

  constexpr bool operator!=(Vec const &v) const { return !(*this == v); }
...
template <typename T> struct Vec<3, T> {
  ...
  constexpr Vec(Vec<3, T> const &value) : x(value.x), y(value.y), z(value.z) {}
  constexpr explicit Vec(T value) : x(value), y(value), z(value.z) {}
  ...
  template <typename U> constexpr Vec<3, T> &operator=(Vec<3, U> const &v) {
    this->x = static_cast<T>(v.x);
    this->y = static_cast<T>(v.y);
  ...
  constexpr Vec<3, T> operator*(const Vec<3, T> &vec) const {
    return Vec<3, T>(this->x * vec.x, this->y * vec.y, this->z * vec.z);
  }
};
...
template <typename T> struct Vec<4, T> {
  ...
  template <typename U> constexpr Vec<4, T> &operator=(Vec<4, U> const &v) {
    this->x = static_cast<T>(v.x);
    this->y = static_cast<T>(v.y);
    this->z = static_cast<T>(v.z);
  ...
  template <typename U> constexpr Vec<4, T> &operator*=(U scalar) {
