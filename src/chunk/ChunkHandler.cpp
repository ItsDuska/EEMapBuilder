...
void chunk::ChunkHandler::update(Vec2 &position) {
  currentChunkCoord = {
      (int)std::floor((position.x / (CHUNK_SIZE * renderSizes.tileSize.x))),
...
void chunk::ChunkHandler::addChunk(Vec2i &chunkPosition) {
  chunks.emplace_back();
...
void chunk::ChunkHandler::setAssetSizes(Vec2 &tileSize, Vec2 &textureSize,
                                        int sheetWidthInTiles) {
  renderSizes.tileSize = tileSize;
  renderSizes.spritePixelSize = textureSize;
  renderSizes.totalChunkSize = {tileSize.x * CHUNK_SIZE,
                                tileSize.y * CHUNK_SIZE};
}

void chunk::ChunkHandler::handleChunks() {
  std::vector<Vec2i> loadingCoords;
  Vec2i tempChunkCord;
  ...
  for (int x = 0; x < renderBonds; x++) {
    for (int y = 0; y < renderBonds; y++) {
      tempChunkCord = {(x + 1) - halfRenderBonds + currentChunkCoord.x,
...
void chunk::ChunkHandler::loadFromFile(const std::string &filename) {
  ...
  inFile.read(reinterpret_cast<char *>(loadedChunks.data()),
              sizeof(ChunkData) * sizes.chunks);

  std::vector<EntityTile> loadedEntities;
  loadedEntities.resize(sizes.entities);
  inFile.read(reinterpret_cast<char *>(loadedEntities.data()),
              sizeof(EntityTile) * sizes.entities);

  std::vector<AnimationTile> loadedAnimations;
  loadedAnimations.resize(sizes.animations);
  inFile.read(reinterpret_cast<char *>(loadedAnimations.data()),
              sizeof(AnimationTile) * sizes.animations);

  std::vector<LayeredStaticTile> loadedLayers;
  loadedLayers.resize(sizes.layeredTiles);
  inFile.read(reinterpret_cast<char *>(loadedLayers.data()),
              sizeof(LayeredStaticTile) * sizes.layeredTiles);
  ...
  for (size_t i = 0; i < sizes.chunks; ++i) {
    ...
    for (size_t indexToBuffer = chunk.entityBuffer.offset;
         indexToBuffer < chunk.entityBuffer.offset + chunk.entityBuffer.count;
         ++indexToBuffer) {
      editorChunk.entities.push_back(loadedEntities[indexToBuffer]);
    }

    for (size_t indexToBuffer = chunk.animatedTileBuffer.offset;
         indexToBuffer <
         chunk.animatedTileBuffer.offset + chunk.animatedTileBuffer.count;
         ++indexToBuffer) {
      editorChunk.animations.push_back(loadedAnimations[indexToBuffer]);
    }

    for (size_t indexToBuffer = chunk.layeredTilesBuffer.offset;
         indexToBuffer <
         chunk.layeredTilesBuffer.offset + chunk.layeredTilesBuffer.count;
         ++indexToBuffer) {
...
void chunk::ChunkHandler::saveToFile(const std::string &filename) {
  ...
  for (const auto &entry : chunkMap) {
    ...
    if (!isChunkEmpty(chunk.rawData.tilemap, TILE_MAP_SIZE)) {
      ...
      outFile.write(reinterpret_cast<const char *>(&chunk.rawData),
                    sizeof(ChunkData));

      allEntities.insert(allEntities.end(), filteredEntities.begin(),
                         filteredEntities.end());
      allAnimations.insert(allAnimations.end(), filteredAnimations.begin(),
                           filteredAnimations.end());
      allLayeredTiles.insert(allLayeredTiles.end(),
                             filteredLayeredTiles.begin(),
                             filteredLayeredTiles.end());
  ...
  outFile.write(reinterpret_cast<const char *>(allEntities.data()),
                sizeof(EntityTile) * sizes.entities);
  outFile.write(reinterpret_cast<const char *>(allAnimations.data()),
                sizeof(AnimationTile) * sizes.animations);
  outFile.write(reinterpret_cast<const char *>(allLayeredTiles.data()),
                sizeof(LayeredStaticTile) * sizes.layeredTiles);
...
chunk::EditorSideChunkData *chunk::ChunkHandler::getEditorSideData(int16_t x,
                                                                   int16_t y) {
  ChunkKey key = combineCoords(x, y);
  auto it = chunkMap.find(key);
...
ChunkData *chunk::ChunkHandler::getChunk(const Vec2i &position) {
  return this->getChunkData(position.x, position.y);
}

bool chunk::ChunkHandler::chunkInMemory(Vec2i &position) {
  const auto result = chunkMap.find(combineCoords(position.x, position.y));
  return result != chunkMap.end();
}

int chunk::ChunkHandler::chunkInActiveMemory(const Vec2i &position) {
  std::vector<Vec2i>::iterator iteratorObj =
      std::find(activeChunks.begin(), activeChunks.end(), position);
...
std::vector<chunk::ChunkCreationTask> &
chunk::ChunkHandler::getChunkCreationTasks() {
...
// Determine if chunks have been loaded and trigger updates in other classes if
// needed.
bool chunk::ChunkHandler::needsUpdate() { return !chunkTaskList.empty(); }

const std::vector<Vec2i> &chunk::ChunkHandler::getActiveChunks() {
  return activeChunks;
}
