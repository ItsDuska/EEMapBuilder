...
void AnimationCache::awake(const Vec2i frameSize, const Vec2i textureSize,
                           std::string &filepath) {
  if (built) {
    return;
  }
...
Vec2i AnimationCache::getAnimationFrame(int animationIndex, int frame) const {
  if (animationIndex == 0) {
    return Vec2i(0, 0);
  }

  if (animationIndex >= startPositions.size()) {
    std::cerr << "INFO: AnimationIndex to frames is too big... Your index is: "
              << animationIndex << "\n";
    animationIndex = 0;
  }

  Vec2i startPos = startPositions[animationIndex];
...
std::vector<Vec2i> *AnimationCache::getStartPositionsPtr() {
...
void AnimationCache::precomputeStartPositions() {
  startPositions.push_back(Vec2i(0, 0)); // NULL Value.
  ...
  for (int i = 1; i < frameCounts.size(); ++i) {
    ...
    startPositions.push_back(Vec2i(x / 16, y / 16));
